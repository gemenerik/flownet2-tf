...
class TinyFlowNet(Net):

    def __init__(self, mode=Mode.TRAIN, debug=False):
        super(TinyFlowNet, self).__init__(mode=mode, debug=debug)

    def model(self, inputs, training_schedule, trainable=True):
        _, height, width, _ = inputs['input_a'].shape.as_list()
        stacked = False
        with tf.variable_scope('TinyFlowNet'):
            ...
            with slim.arg_scope([slim.conv2d, slim.conv2d_transpose],
                ...
                with slim.arg_scope([slim.conv2d], weights_regularizer=weights_regularizer):
                    with slim.arg_scope([slim.conv2d], stride=2):
                        conv_1 = slim.conv2d(pad(concat_inputs, 3), 24, 7, scope='conv1')
                        conv_2 = slim.conv2d(pad(conv_1, 2), 48, 5, scope='conv2')
                        conv_3 = slim.conv2d(pad(conv_2, 2), 96, 5, scope='conv3')

                    conv3_1 = slim.conv2d(pad(conv_3), 96, 3, scope='conv3_1')
